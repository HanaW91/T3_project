...
metric_specs <- list(
  list(metric = "recall", label = "Recall"),
  list(metric = "precision", label = "Precision")
)
...
plot_metric_grid <- function(summary_results, result_set, method_spec, metric_spec) {
  ...
  if (nrow(plot_data) == 0) {
    warning("No appendix plot data found for ", result_set$id, " / ", method_spec$method_id)
  ...
  graphics::mtext(
    paste0(method_spec$method_label, " ", metric_spec$label, " across rarity, noise, and correlation"),
    outer = TRUE,
    side = 3,
    line = 4.2,
    cex = 1.45,
...
for (result_set in result_sets) {
  ...
  for (metric_spec in metric_specs) {
    summary_results <- summarise_metric(raw_results, metric_spec$metric)

    for (method_spec in method_specs) {
      created_file <- plot_metric_grid(
...
message("Created appendix recall/precision plot files:")
