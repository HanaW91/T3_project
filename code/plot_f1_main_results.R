...
#
# Output:
#   standard-setting F1 plots in plots/main
#   high-dimensional F1 plots in plots/appendix/highdim
# Recall and precision-only plots are generated by plot_appendix_recall_precision.R.
...
main_plot_dir <- file.path("plots", "main")
appendix_plot_dir <- file.path("plots", "appendix", "highdim")
for (dir in c(main_plot_dir, appendix_plot_dir)) {
  if (!dir.exists(dir)) {
...
plot_f1_grid <- function(summary_results, result_set, method_spec) {
  ...
  output_dir <- if (startsWith(result_set$id, "highdim_")) {
    appendix_plot_dir
  } else {
    main_plot_dir
  }
