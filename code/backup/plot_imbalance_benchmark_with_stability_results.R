...
plot_rarity_severity_boxplots <- function(
    binary_fraction = 0.5,
    pk_imbalance_fraction = 0.2,
    ev_xy_to_plot = 0.5,
    ev_xx_to_plot = 0.6,
    file_name = "rarity_severity_best_scaling_boxplots.png") {
  rarity_levels <- c(0.5, 0.2, 0.1, 0.05)
  metric_specs <- data.frame(
    metric = c("f1_score", "recall", "precision"),
    label = c("F1 Score", "Recall", "Precision"),
    stringsAsFactors = FALSE
  )
      ...
      (
        (
          raw_results$binary_top_fraction == 0.5 &
            raw_results$pk_imbalance_fraction == 0
        ) |
          (
            raw_results$binary_top_fraction != 0.5 &
              raw_results$pk_imbalance_fraction == pk_imbalance_fraction
          )
      ) &
      raw_results$binary_top_fraction %in% rarity_levels,
  ]
  ...
  best_scaling_scores <- aggregate(
    f1_score ~ algorithm + scaling_method,
    data = plot_data,
    FUN = function(x) mean(x, na.rm = TRUE)
  )
  best_scaling_scores$algorithm <- factor(
    best_scaling_scores$algorithm,
    levels = algorithm_levels
  )
  best_scaling_scores <- best_scaling_scores[
    order(
      best_scaling_scores$algorithm,
      -best_scaling_scores$f1_score,
      match(best_scaling_scores$scaling_method, scaling_levels)
    ),
  ]
  best_scaling_rows <- best_scaling_scores[
    !duplicated(best_scaling_scores$algorithm),
  ]
  ...
  layout_matrix <- matrix(seq_len(12), nrow = 3, ncol = 4, byrow = TRUE)
  layout_matrix <- rbind(layout_matrix, rep(13, 4))

  graphics::layout(layout_matrix, heights = c(1, 1, 1, 0.24))
  graphics::par(mar = c(4.2, 4.9, 2.1, 1.2), oma = c(0, 0, 5.4, 0))

  short_algorithm_labels <- c(
    cv_lasso_min = "CV min",
    cv_lasso_1se = "CV 1se",
    stability_lasso_ncut_0 = "Stab 0",
    stability_lasso_ncut_3 = "Stab 3"
  )

  best_labels <- character(0)
  best_colours <- character(0)
  best_symbols <- integer(0)

  for (algorithm in algorithm_levels) {
    best_row <- best_scaling_rows[
      as.character(best_scaling_rows$algorithm) == algorithm,
    ]
    ...
    best_colours <- c(best_colours, combo_colours[combo_key])
    best_symbols <- c(best_symbols, scaling_symbols[best_scaling])
  }
  ...
  for (metric_index in seq_len(nrow(metric_specs))) {
    ...
    for (rarity_index in seq_along(rarity_levels)) {
      rarity_level <- rarity_levels[rarity_index]
      panel_data <- plot_data[plot_data$binary_top_fraction == rarity_level, ]

      values_by_algorithm <- lapply(algorithm_levels, function(algorithm) {
        best_row <- best_scaling_rows[
          as.character(best_scaling_rows$algorithm) == algorithm,
        ]
        ...
        best_scaling <- as.character(best_row$scaling_method[1])
        algorithm_data <- panel_data[
          panel_data$algorithm == algorithm &
            panel_data$scaling_method == best_scaling,
        ]
        algorithm_data[[metric_name]]
      })

      graphics::boxplot(
        values_by_algorithm,
        ylim = c(0, 1),
        names = short_algorithm_labels[algorithm_levels],
        col = grDevices::adjustcolor(best_colours, alpha.f = 0.58),
        border = best_colours,
        outline = TRUE,
        ylab = if (rarity_index == 1) metric_label else "",
        main = if (metric_index == 1) {
          paste0("bt = ", rarity_level)
        } else {
          ""
        },
        cex.lab = 1.05,
  ...
  graphics::mtext(
    "Columns show rare-category level; rows show metrics; each algorithm uses its F1-best scaling",
    outer = TRUE,
    side = 3,
    line = 1.8,
    cex = 0.92
...
created_rarity_boxplot_files <- unlist(
  lapply(seq_len(nrow(available_slices)), function(i) {
    ev_xy_to_plot <- available_slices$ev_xy[i]
    ev_xx_to_plot <- available_slices$ev_xx[i]

    plot_rarity_severity_boxplots(
      binary_fraction = binary_fraction_to_plot,
      pk_imbalance_fraction = 0.2,
      ev_xy_to_plot = ev_xy_to_plot,
      ev_xx_to_plot = ev_xx_to_plot,
      file_name = paste0(
        ...
        "_xx",
        format_file_value(ev_xx_to_plot),
        "_pk0_2_best_scaling_rarity_severity_boxplots.png"
      )
    )
  }),
  use.names = FALSE
