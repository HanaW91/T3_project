# Noise-focused benchmark using fake-generated regression data.
# Main check: see how performance changes as the signal strength changes.
...
run_one_noise_scenario <- function(seed,
                                   scaling_methods,
                                   binary_fraction,
                                   binary_top_fraction,
                                   pk_imbalance_fraction,
...
run_noise_benchmark <- function(
    ...
    pk_imbalance_fraction = 0.2,
    nu_xy = 0.10,
    ev_xy_values = seq(0.1, 0.9, by = 0.1),
    ev_xx = 0.4,
    nfolds = 5,
    nlambda = 50) {
  grid <- expand.grid(
  ...
  for (i in seq_len(nrow(grid))) {
    ...
    results[[i]] <- run_one_noise_scenario(
      seed = grid$seed[i],
...
print(head(noise_results))

noise_summary <- aggregate(
...
utils::write.csv(
  noise_results,
  file = file.path("results", "noise_benchmark_results.csv"),
  row.names = FALSE
)

utils::write.csv(
  noise_summary,
  file = file.path("results", "noise_benchmark_summary.csv"),
  row.names = FALSE
)
