...
group_specs <- data.frame(
  group_label = c("All binary", "Continuous", "Rare binary", "Non-rare binary"),
  recall_column = c(
    "binary_recall",
    "continuous_recall",
    "rare_binary_recall",
...
summarise_group_metric <- function(data, value_column) {
  split_keys <- interaction(
    data$algorithm,
    data$scaling_method,
    data$binary_fraction,
  ...
  summary_rows <- lapply(pieces, function(piece) {
    ...
    data.frame(
      ...
      ev_xx = piece$ev_xx[1],
      mean = mean_value,
      ci_low = max(0, mean_value - ci_width, na.rm = TRUE),
      ci_high = min(1, mean_value + ci_width, na.rm = TRUE),
      n = n_values
    )
  })
...
plot_group_panel <- function(panel_data,
                             ev_xx,
                             group_label,
                             metric_label,
                             show_y_label = FALSE,
  ...
  panel_data <- panel_data[
    panel_data$ev_xx == ev_xx &
      panel_data$group_label == group_label,
  ]
  x_values <- sort(unique(raw_results$ev_xy))
...
plot_group_correlation_for_binary_fraction <- function(binary_fraction,
                                                       metric_name,
                                                       metric_label,
                                                       file_name) {
  needed_columns <- if (metric_name == "recall") {
    group_specs$recall_column
  } else {
    group_specs$f1_column
  }

  missing_group_columns <- setdiff(needed_columns, names(raw_results))

  if (length(missing_group_columns) > 0) {
    warning(
      "Skipping ",
      metric_label,
      " group plot because the results file is missing: ",
      paste(missing_group_columns, collapse = ", "),
      ". Re-run source(\"code/run_correlation_benchmark.R\") first."
    )
    return(invisible(NULL))
  }

  group_plot_data <- lapply(seq_len(nrow(group_specs)), function(i) {
    value_column <- if (metric_name == "recall") {
      group_specs$recall_column[i]
    } else {
      group_specs$f1_column[i]
    }

    summary_data <- summarise_group_metric(raw_results, value_column)
    summary_data$group_label <- group_specs$group_label[i]
    summary_data
  })
  group_plot_data <- do.call(rbind, group_plot_data)
  group_plot_data <- group_plot_data[
    group_plot_data$binary_fraction == binary_fraction,
  ]
  ...
  ev_xx_values <- sort(unique(group_plot_data$ev_xx))
  output_file <- file.path(plot_dir, file_name)
  ...
  panel_count <- length(ev_xx_values) * nrow(group_specs)
  layout_matrix <- matrix(
    seq_len(panel_count),
    nrow = length(ev_xx_values),
    ncol = nrow(group_specs),
    byrow = TRUE
  )
  layout_matrix <- rbind(
    layout_matrix,
    rep(panel_count + 1, nrow(group_specs))
  )
  ...
  for (row_index in seq_along(ev_xx_values)) {
    for (group_index in seq_len(nrow(group_specs))) {
      plot_group_panel(
        panel_data = group_plot_data,
        ev_xx = ev_xx_values[row_index],
        group_label = group_specs$group_label[group_index],
        metric_label = metric_label,
        show_y_label = group_index == 1,
        show_x_label = row_index == length(ev_xx_values)
      )
    }
  }
  ...
  graphics::mtext(
    paste0(
      metric_label,
      " by Predictor Group, Split by Correlation - ",
      format_binary_fraction(binary_fraction)
    ),
    outer = TRUE,
    side = 3,
  ...
  graphics::mtext(
    "Rows show predictor correlation; columns split selection performance by predictor type and rarity",
    outer = TRUE,
    side = 3,
    line = 1.3,
    cex = 1.08
...
created_group_files <- unlist(lapply(binary_fractions_to_plot, function(binary_fraction) {
  file_stub <- if (binary_fraction == 1) {
    "all_binary"
  } else {
    paste0(
  ...
  c(
    plot_group_correlation_for_binary_fraction(
      binary_fraction = binary_fraction,
      metric_name = "recall",
      metric_label = "Recall",
      file_name = paste0(
        "scaling_by_correlation_group_recall_",
        file_stub,
        ".png"
      )
    ),
    plot_group_correlation_for_binary_fraction(
      ...
      file_name = paste0(
        "scaling_by_correlation_group_f1_",
        file_stub,
        ".png"
      )
    )
...
created_group_files <- created_group_files[!is.na(created_group_files)]

if (length(created_group_files) > 0) {
  message("Created group plot files:")
  for (created_file in created_group_files) {
    message(" - ", created_file)
  }
}
