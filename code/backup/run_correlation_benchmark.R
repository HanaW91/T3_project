...
summary_value_columns <- names(correlation_results)[
  vapply(correlation_results, is.numeric, logical(1))
]
summary_value_columns <- setdiff(
  summary_value_columns,
  c(summary_group_columns, "seed", "n", "p", "lambda", "elapsed_seconds")
)

correlation_summary <- aggregate(
  correlation_results[summary_value_columns],
  by = correlation_results[summary_group_columns],
